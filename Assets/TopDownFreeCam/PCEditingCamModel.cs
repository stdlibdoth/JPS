    ...
    ,IEditCamPitch<float>
{
    ...
    private void Awake()
    {
        CameraControlInput camInput = new CameraControlInput();
        camInput.PCEditingCam.MiddleBtnPress.performed += OnPanningStart;
        camInput.PCEditingCam.MiddleBtnRelease.performed += OnPanningCancel;
        camInput.PCEditingCam.RightBtnPress.performed += OnRotateStart;
        camInput.PCEditingCam.RightBtnRelease.performed += OnRotateCancel;
        camInput.PCEditingCam.MouseDelta.performed += OnMouseDelta;
        camInput.PCEditingCam.Scroll.performed += OnZooming;
        camInput.PCEditingCam.SpaceKeyPress.performed += OnCamReset;
        camInput.PCEditingCam.Enable();
    }
    ...
    private void OnMouseDelta(InputAction.CallbackContext arg)
    {
        if (m_rotatingFlag)
        {
